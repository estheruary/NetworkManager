                                     ...
                                     char **out_to_free)
{
	...
	gboolean escape_backslash_as_needed_before_space;
	...
	if (NM_FLAGS_HAS (flags, NM_UTILS_ESCAPED_TOKENS_ESCAPE_FLAGS_ESCAPE_BACKSLASH_ALWAYS)) {
		...
		escape_backslash_as_needed_before_space = FALSE;
	} else {
		escape_backslash_as_needed =    NM_FLAGS_HAS (flags, NM_UTILS_ESCAPED_TOKENS_ESCAPE_FLAGS_ESCAPE_BACKSLASH_AS_NEEDED)
		                             && !_char_lookup_has (ch_lookup, '\\');

		/* if the caller wants to escape leading/trailing whitespace, we also want to escape all
		 * backslashes before a space. */
		escape_backslash_as_needed_before_space =    escape_backslash_as_needed
		                                          && NM_FLAGS_ANY (flags,   NM_UTILS_ESCAPED_TOKENS_ESCAPE_FLAGS_ESCAPE_LEADING_SPACE
		                                                                  | NM_UTILS_ESCAPED_TOKENS_ESCAPE_FLAGS_ESCAPE_TRAILING_SPACE);
	}
	...
	for (i = 0; str[i] != '\0'; i++) {
		if (_char_lookup_has (ch_lookup, str[i]))
			n_escapes++;
		else if (   escape_backslash_as_needed
		         && str[i] == '\\'
		         && (   _char_lookup_has (ch_lookup, str[i + 1])
		             || NM_IN_SET (str[i + 1], '\0', '\\')
		             || (   escape_backslash_as_needed_before_space
	...
	alloc_len = str_len + n_escapes + ((gsize) escape_leading_space) + ((gsize) escape_trailing_space) + 1;
